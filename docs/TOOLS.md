...
Rewrite a PNG/JPEG/WebP container keeping only what's needed to display the pixels — drops EXIF,
XMP, IPTC, comments and **C2PA content-credential manifests** (AI-provenance markers). Lossless:
pixel data is copied verbatim, never re-compressed. Images generated by this tool are **already
stripped automatically on save** (`GPT_IMAGE_KEEP_METADATA=1` to opt out) — use this tool for
images from elsewhere. Note: this removes *metadata* provenance only; it cannot remove a watermark
encoded in the pixels themselves.

| param | type | notes |
|---|---|---|
| `image_path` | string | Image to scrub. **Required.** |
| `output_path` | string | Default: overwrite in place (pixels unchanged). |
...
| `--strip-metadata <path>` | Scrub EXIF/XMP/C2PA provenance, lossless (default in place; `-o` to write elsewhere). |
...
| `GPT_IMAGE_KEEP_METADATA` | off | `1` = keep the model's original container bytes (EXIF/XMP/C2PA provenance is stripped on save by default). |
