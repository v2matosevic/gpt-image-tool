...
function artZoneFor(pos: OverlayPosition): string {
  const [v, h] = pos.split("-").length === 2 ? pos.split("-") : ["center", "center"];
...
export async function createSocialCard(input: SocialCardInput): Promise<SocialCardResult> {
  ...
  const plate: GenerateOutput = await generateImage({
    ...
    outputPath: input.outputPath ? input.outputPath.replace(/(\.[a-z]+)?$/i, "-plate.png") : undefined,
  });
  ...
  if (input.subline) {
    const [v] = headlinePos.split("-");
    blocks.push({
      text: input.subline,
      position: (v === "bottom" ? headlinePos.replace("bottom", "top") : headlinePos.replace("top", "bottom")) as OverlayPosition,
