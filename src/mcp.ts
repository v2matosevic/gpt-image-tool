...
//   strip_image_metadata  scrub EXIF/XMP/C2PA provenance from an existing image (lossless)
...
server.registerTool(
  ...
  {
    title: "Strip provenance metadata from an image (EXIF/XMP/C2PA)",
    description:
      "Rewrite a PNG/JPEG/WebP container keeping only what's needed to display the pixels — " +
      "removes EXIF, XMP, IPTC, comments and C2PA content-credential manifests (AI-provenance " +
      "markers). Lossless: pixel data is copied verbatim, never re-compressed. NOTE: images " +
      "generated by this tool are already stripped automatically on save (GPT_IMAGE_KEEP_METADATA=1 " +
      "to opt out) — use this for images from elsewhere.",
    inputSchema: {
      image_path: z.string().describe("Image to scrub (png/jpeg/webp)."),
      output_path: z.string().optional().describe("Where to save. Default: overwrite in place (pixels are unchanged)."),
    },
  },
  async (a) => {
    try {
      const buf = await readFile(a.image_path);
      const before = listMetadataBlocks(buf);
      const clean = stripImageMetadata(buf);
      const out = a.output_path ?? a.image_path;
      await writeFile(out, clean);
      const removed = buf.length - clean.length;
      const after = listMetadataBlocks(clean);
      const dropped = before.filter((b) => !after.includes(b));
      const text =
        `Metadata stripped → ${out} (${removed} bytes of metadata removed).` +
        (dropped.length ? `\nDropped blocks: ${[...new Set(dropped)].join(", ")}` : "\nNo metadata blocks found — image was already clean.");
      return { content: [{ type: "text", text }] } as any;
    } catch (e) {
      return fail(e);
    }
